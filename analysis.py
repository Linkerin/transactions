...
class Transactions:
    ...
    def duplicates(self, data):
        ...
        duplicate_filt = ((data['Статья ДДС'] == 'Перечисление ДС на брокерские счета') & \
                         (data['Получатель.ИНН'] == '7750004023') is False) & \
                         ((data['Статья ДДС'] == 'Перечисление ДС на ведение хозяйственной деятельности внутри организации') & \
                         (data['Получатель.ИНН'] == '7729355614') is False) & \
                         ...
                         (data['Получатель.ИНН'] == '7702077840')) & \
                         ((data['Получатель'] == data['Организация']) is False)
